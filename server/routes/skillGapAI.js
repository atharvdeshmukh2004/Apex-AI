...
// //
// // 🔥 SAFE GEMINI CALL (WITH FALLBACK + RETRY)
// //
...
// //
// // 🔥 1. GET SKILLS (REAL + DYNAMIC)
// //
// router.post("/get-skills", async (req, res) => {
//   try {
//     const { career, education, interests, experience } = req.body;
...
//     const prompt = `
// You are a senior hiring manager in ${career} industry.
...
// TASK:
// Generate REAL skills required for THIS role.

// STRICT RULES:
// - Skills MUST be specific to ${career}
// - Include tools, frameworks, technologies
// - NO generic skills like communication/teamwork
// - Output must vary for different careers
...
//     // 🔥 SMART FALLBACK (DYNAMIC BASED ON CAREER)
//     if (skills.length === 0) {
//       const baseSkillsMap = {
...
//       const matched =
//         Object.keys(baseSkillsMap).find((k) => key.includes(k)) || "";

//       const fallbackSkills = baseSkillsMap[matched] || [
//         `${career} fundamentals`,
//         `${career} tools`,
//         `${career} projects`,
//       ];

//       skills = fallbackSkills.map((s) => ({
//         skill: s,
//         requiredLevel: 7,
//         category: "core",
//       }));
...
//         error: "career, skills, userSkills required",
//       });
//     }

//     const requiredSkills = skills
//       .map((s) => `${s.skill} (${s.requiredLevel}/10)`)
//       .join("\n");

//     const userSkillList = Object.entries(userSkills)
//       .map(([k, v]) => `${k}: ${v}/10`)
//       .join("\n");
...
//       data = {
//         summary: `Gap analysis for ${career}`,
//         analysis: skills.map((s) => {
//           const userLevel = userSkills[s.skill] || 2;
//           const gap = s.requiredLevel - userLevel;
...
//             gap,
//             level: gap > 5 ? "High" : gap > 2 ? "Medium" : "Low",
//             reason: `Below expected level for ${career}`,
//             action: `Practice ${s.skill} via projects and real-world tasks`,
//           };
//         }),
//         priority: skills
...
//     return res.status(500).json({ error: "Failed to analyze gap" });
//   }
// });
